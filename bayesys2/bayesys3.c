...
//  The basesys library incorporated in BayeSys provides Flux procedures for
...
//=============================================================================
//
// The "Massive Inference" (basesys) option is provided for applications where
...
// Common->basesys defines the shape P(z) of the prior according to
//
...
//
// according to the "hundreds" digit of the switch Common->basesys.
//
// For example, Common->basesys =  1  is Gaussian data with "positive" prior;
//              Common->basesys = 101  is Poisson data with "positive" prior.
//=============================================================================
// History:
// basesys1 v1.12-2.36  1998-2000
// BayeSys1 code        1999-2000
// BayeSys2 v1.02-1.05  1 May - 10 Sep 2001
// BayeSys3 v1.01-1.32  2 Jan 2002 - 4 Feb 2003
//          v2.00      10 Feb 2003
//          v2.01      11 Jul 2003  Fixed-q0 option to fix FluxUnit0 in basesys
//          v2.02      19 Jul 2003  basesys options extended to allow flux=0
//          v2.10      12 Aug 2003  basesys needs FluxUnit0 from user
//          v2.20      14 Aug 2003  Remove any overlay in basesys user's "bits"
//          v3.00      20 Aug 2003  Poisson data as basesys option
//          v3.01      12 Sep 2003  basesys control of flux=0 by ProbON
...
// basesys application procedures for user Footprints
...
// basesys application procedures for quadratic chisquared
...
// basesys application procedures for quadratic probabilities
...
// basesys application procedures for Poisson probabilities
...
// Check input parameters
    ...
    if( Valency )
    {
        if( Common->basesys !=   0 && Common->basesys !=   1
         && Common->basesys !=   2 && Common->basesys !=   3
         && Common->basesys != 100 && Common->basesys != 101 )
                                      return E_BASESYS_PARMS;
        if(   Common->Ndata < 0 )     return E_BASESYS_PARMS;
        if( ! Common->Data )          return E_BASESYS_PARMS;
        if( ! Common->Acc )           return E_BASESYS_PARMS;
        if( Common->ProbON <= 0.0 )   return E_BASESYS_PARMS;
        if( Common->basesys >= 100 )
            for( k = 0; k < Common->Ndata; k++ )
            {
               if( Common->Acc[k] < 0. || Common->Data[k]+Common->Acc[k] < 0. )
                   return E_BASESYS_DATA;
...
Exit:
// Free memory Dosn't work
//    BayesFree(Common, Objects, Links, Ocean);
//    FREE(Links)	printf("Hello1\n");
//    FREE(ntable)
//   FREE(ctable)
//    FREE(Ltable)
    return CALLvalue;
...
// Function:  BayesAlloc
// 
// Purpose:   Allocate memory for BayeSys (and basesys)
// 
...
// Function:  BayesInit
// 
// Purpose:   Initialise BayeSys (and basesys)
// 
...
// Function:  BayesFree
// 
// Purpose:   Free allocated memory for BayeSys (and basesys)
// 
...
Node*      Links)      //   O  new trees
{
    ...
    Object->Natoms = 0;
    CALL( Empty(Common, Object) )
    Object->reset = 0;                            // keep basesys fluxes
    for( j = 0; j < Natoms; j++ )
        CALL( Insert1(Common, Object, Link) )
...
// Function:  FluxAlloc
// 
// Purpose:   Allocate memory for basesys
// 
...
// Function:  FluxInit
// 
// Purpose:   Initialise system for basesys
// 
...
Node*      Links)        //  (O) empty on exit
{
    ...
    int        basesys  = Common->basesys;      // I    type of data
...
// New flux unit
    ...
    for( i = 0; i < Ndata; i++ )
    {
        if( basesys >= 100 )         // Poisson (approx is OK here)
        {
...
// Function:  FluxFree
// 
// Purpose:   Free memory for basesys
// 
// History:   JS       10 Feb 2003
//-----------------------------------------------------------------------------
static 
...
ObjectStr* Object)    // I(O) sample object
{
    int      basesys = Common->basesys;    // basesys switches
    int      Ndata   = Common->Ndata;      // I    # data
    double*  Data    = Common->Data;       // I    Data            [Ndata]
    double*  Acc     = Common->Acc;        // I    Accuracies      [Ndata]
...
// Marginal likelihood
        *dLtry = GaussTry1(basesys, cool, q, ProbON, Valency, g1, A11);
    }

// Poisson
    else
        CALL( PoissTry1(basesys, cool, q, ProbON, Mock, Data, Acc, Counts,
...
// Marginal likelihoods
        GaussTry2(basesys, cool, q, ProbON, Valency,
                  g1, g2, A11, A12, A22, Xindex, dLtry1, dLtry2);
    }

// Poisson
    else
        CALL( PoissTry2(basesys, cool, q, ProbON, Mock, Data,Acc,Counts, Foot,
...
ObjectStr* Object)      // I O  updated sample object
{
    ...
    double*   g1      = Object->g1;         // grad chisquared        [Valency]
    double*   A11     = Object->A11;        // grad grad chisquared   [Valency]
    int       reset   = Object->reset;      // Reset inserted basesys fluxes?
    double    q       = Object->FluxUnit;   // Unit of flux
...
// Optionally reset Flux
        if( reset )
            GaussInsert1(basesys, Rand, cool, q, ProbON, Valency,
                        g1, A11, Flux);
// Modify Lhood
...
// Poisson
    ...
    {
        if( reset )
            CALL( PoissInsert1(basesys, Rand, cool, q, ProbON, Mock, Data, Acc,
...
// Construct quadratic chisquared
        SetGrad2(Mock, Data, Acc, Foot, Valency,
                 ...
                 g1, g2, A11, A12, A22, Xindex);
// Reset Fluxes
        GaussInsert2(basesys, Rand, cool, q, ProbON, Valency,
...
// Poisson
    ...
    {
        CALL( PoissInsert2(basesys, Rand, cool, q, ProbON,
...
double*  A11)          // I    grad grad chisquared    [Valency]
{
    ...
    for( v = 0; v < Valency; v++ )
        dL += Gauss1Marginal(basesys, cool, q, ProbON, g1[v], A11[v]);
    return dL;
}
...
// Trial Lhood increment
    for( v1 = 0; v1 < Valency; v1++ )
        *dLtry1 += Gauss1Marginal(basesys, cool, q, ProbON, g1[v1], A11[v1]);
...
// Trial Lhood increment
    for( v1 = 0; v1 < Valency; v1++ )
    {
        ...
        if( v2 >= 0 )                     // pick up Atom1/Atom2 cross term...
            *dLtry2 += Gauss2Marginal(basesys, cool, q, ProbON,
                                 g1[v1], g2[v2], A11[v1], A12[v1], A22[v2]);
        else                              // ... or pick up lone Atom1
            *dLtry2 += Gauss1Marginal(basesys, cool,q,ProbON, g1[v1],A11[v1]);
    }
    for( v2 = 0; v2 < Valency; v2++ )
    {
        for( v1 = 0; v1 < Valency; v1++ )
        ...
        if( v1 == Valency )               // remaining lone Atom2
            *dLtry2 += Gauss1Marginal(basesys, cool,q,ProbON, g2[v2],A22[v2]);
    }
}
...
double*  Flux)       //   O  fluxes                  [Valency]
{
    int  v;
    for( v = 0; v < Valency; v++ )
        Flux[v] = Gauss1Sample(basesys, Rand, cool, q, ProbON, g1[v], A11[v]);
}
...
double*  Flux2)      //   O  fluxes of atom2         [Valency]
{
    ...
    for( v1 = 0; v1 < Valency; v1++ )
    {
        v2 = Xindex[v1];
        if( v2 >= 0 )
            Gauss2Sample(basesys, Rand, cool, q, ProbON,
                         g1[v1], g2[v2], A11[v1], A12[v1], A22[v2],
                         &Flux1[v1], &Flux2[v2]);
        else
            Flux1[v1] = Gauss1Sample(basesys, Rand, cool, q, ProbON,
                                     g1[v1], A11[v1]);
    }
    for( v2 = 0; v2 < Valency; v2++ )
    {
        for( v1 = 0; v1 < Valency; v1++ )
            if( Xindex[v1] == v2 )
                break;
        if( v1 == Valency )
            Flux2[v2] = Gauss1Sample(basesys, Rand, cool, q, ProbON,
                                     g2[v2], A22[v2]);
    }
}
...
int      basesys,      // I    basesys prior (0,1,2,3)
double   cool,         // I    Annealing coefficient
double   q,            // I    Flux unit
double   ProbON,       // I    Pr(individual flux != 0)
double   g,            // I    gradient
double   A)            // I    curvature
{
    ...
    p = gauss1marginal(basesys, cool, q, g, A);
    if( ProbON < 1.0 )
    {
        ProbOFF = 1.0 - ProbON;
...
double   A22)          // I    curvature
{
    ...
    p12 = gauss2marginal(basesys, cool, q, g1, g2, A11, A12, A22);
    if( ProbON < 1.0 )
    {
        ProbOFF = 1.0 - ProbON;
        p1  = gauss1marginal(basesys, cool, q, g1, A11);
        p2 = gauss1marginal(basesys, cool, q, g2, A22);
...
double   A)          // I    curvature
{
    ...
    {
        ProbOFF = 1.0 - ProbON;
        p = log(ProbON) + cool * gauss1marginal(basesys, cool, q, g, A);
        if( log(Randouble(Rand)) > p - PLUS(log(ProbOFF), p) )
            k = 0;
    }
    return  k ? gauss1sample(basesys, Rand, cool, q, g, A) : 0.0;
}
    ...
    *Flux1 = *Flux2 = 0.0;
    if( ProbON < 1.0 )
    {
        ...
        p1  = log(ProbOFF) + log(ProbON)
             + cool * gauss1marginal(basesys, cool, q, g1, A11);
        p2  = log(ProbOFF) + log(ProbON)
             + cool * gauss1marginal(basesys, cool, q, g2, A22);
        p12 = log(ProbON) + log(ProbON)
             + cool * gauss2marginal(basesys, cool, q, g1, g2, A11, A12, A22);
        a = PLUS(p, p1);
        b = PLUS(p2, p12);
        c = PLUS(a, b);
        p1  = exp(p1 - c);
    ...
    if( k == 1 )
        *Flux1 = gauss1sample(basesys, Rand, cool, q, g1, A11);
    if( k == 2 )
        *Flux2 = gauss1sample(basesys, Rand, cool, q, g2, A22);
    if( k == 3 )
        gauss2sample(basesys, Rand, cool, q, g1,g2, A11,A12,A22, Flux1, Flux2);
}
...
static 
double gauss1marginal( //   O  value, calculated OK even if cool=0
int     basesys,       // I    Prior (0=monkeys,1=pos,2=posneg,3=gauss)
double  cool,          // I    Annealing coefficient
double  q,             // I    Flux unit
double  g,             // I    gradient
double  A)             // I    curvature
...
//
// History:   JS    2 Jan 2002
//                 17 Dec 2002   basesys=3 debugged
//                 19 Jul 2003   "switch" recoding
...
int      basesys,  // I    100=monkeys, 101=positive, add 10 for z=0
double   cool,     // I    Annealing coefficient
double   q,        // I    Flux unit
double   ProbON,   // I    Pr(individual flux != 0)
double*  Mock,     // I    Mock data                 [Ndata]
...
double*  dLtry)    //   O  (1/cool) INT dprior (L/L0)^cool
{
    ...
    {
        CALL( Poiss1Marginal(basesys, cool, q, ProbON, Mock, Data, Acc, Counts,
...
// Trial Lhood increments
    for( k1 = v1 = 0; v1 < Valency; k1 += nbits[v1++] )
    {
        ...
        if( v2 >= 0 )                     // pick up Atom1/Atom2 cross term...
        {
            ...
            CALL( Poiss2Marginal(basesys, cool, q, ProbON,
                                 Mock, Data, Acc, Counts, Foot,
        ...
        {
            Poiss1Marginal(basesys, cool, q, ProbON, Mock, Data, Acc, Counts,
...
double*  Flux)     //   O  Sample fluxes           [Valency]
{
    ...
    for( k = v = 0; v < Valency; k += nbits[v++] )
        CALL( Poiss1Sample(basesys, Rand, cool, q, ProbON, Mock, Data, Acc,
...
double*  Flux2)    //   O  Fluxes of 2nd atom      [Valency]
{
    ...
    for( k1 = v1 = 0; v1 < Valency; k1 += nbits[v1++] )
    {
        ...
        {
            for( k2 = i = 0; i < v2; k2 += nbitx[i++] ) ;
            CALL( Poiss2Sample(basesys, Rand, cool, q, ProbON,
                               Mock, Data, Acc, Counts, Foot,
                               nbits[v1], &ibits[k1], &zbits[k1],
                               nbitx[v2], &ibitx[k2], &zbitx[k2],
                               &Flux1[v1], &Flux2[v2]) )
            ...
            CALL( Poiss1Sample(basesys, Rand, cool, q, ProbON,
...
double*  dLtry)    //   O  (1/cool) INT dprior (L/L0)^cool
{
    ...
    CALL( Poisson1(basesys, NULL, cool, q, Mock, Data, Acc, Counts,
                   nbits, ibits, zbits, &p) )
    if( ProbON < 1.0 )
    {
        ProbOFF = 1.0 - ProbON;
...
double*  dLtry2)   //   O  (1/cool) INT dprior[2] (L/L0)^cool
{
    ...
    CALL( Poisson2(basesys, NULL, cool, q, Mock, Data, Acc, Counts, Foot,
...
double*  Flux)     //   O  Sample flux z
{
    ...
    *Flux = 0.0;
    ...
    if( k )
        CALL( Poisson1(basesys, Rand, cool, q, Mock, Data, Acc, Counts,
...
double*  Flux2)    //   O  Flux y of 2nd atom
{
    ...
    *Flux1 = *Flux2 = 0.0;
    ...
    {
        ProbOFF = 1.0 - ProbON;
        CALL( Poisson2(basesys, NULL, cool, q, Mock, Data, Acc, Counts,
    ...
    if( k == 3 )
        CALL( Poisson2(basesys, Rand, cool, q, Mock, Data, Acc, Counts, Foot,
...
int      basesys,  // I    100=monkeys, 101=positive
...
// Get sum1 (usable for dL1)
            ...
            for( i = 0; i <= npoly0; i++ )
            {
                ...
                for( k = 0; k <= npoly1; k++ )
                {
                    ...
                    x *= 1.0 + i / (k + 1.0);
                    if( x > BIG )
                        return E_BASESYS_COUNTS;  // incipient overflow
                }
                for( j = 0; i + j <= npoly0; j++ )
                    poly0[i][j] *= weight;
                sum1 += poly0[i][0];
